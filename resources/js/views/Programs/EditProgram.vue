    ...
    >
        ...
        <v-row>
            ...
            <v-col>
                ...
                <img
                    v-if="program.focus_image_url"
                    :src="returnImagePath(program.focus_image_url)"
                    alt=""
            ...
            <v-col>
                ...
                <img
                    v-if="program.based_image_url"
                    :src="returnImagePath(program.based_image_url)"
                    alt=""
        ...
        <v-row>
            ...
            <v-col>
                ...
                <img
                    ...
                    :src="returnImagePath(program.featured_image)"
                    alt=""
        ...
        <v-btn color="primary" @click="addWeek()" class="mt-5">
            Add Week
        </v-btn>
        <v-btn
            color="primary"
...
const getProgramInfo = async (id) => {
    try {
        const { data } = await axios.get(`/api/web/get-package/${id}`);
        program.value = data;
        overview.value.commands.setContent(data.overview);
        introduction.value.commands.setContent(data.introduction);
        if (data.weeks.length > 0) {
            selectedDays.value = data.weeks;
        }
    } catch (error) {
        console.error(error);
    }
